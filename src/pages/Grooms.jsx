...
const Groom = () => {
  ...
  // ✅ 1. CACHE LOADER: if componet mount please check the catche
  useEffect(() => {
    const cachedData = localStorage.getItem("cached_grooms");
    const cachedTimestamp = localStorage.getItem("grooms_cache_timestamp");
    ...
    if (cachedData && isCacheFresh) {
      ...
      console.log("📦 Loaded grooms from cache");
    }
  }, []);
  ...
  useEffect(() => {
    if (users.length > 0) {
      setAllUsers((prev) => {
        ...
        // Save to cache
        localStorage.setItem("cached_grooms", JSON.stringify(newAllUsers));
        localStorage.setItem("grooms_cache_timestamp", Date.now());
      ...
      <section className="flex-1 p-4 md:p-6 mt-5">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="text-center mt-4">
            <button
              onClick={() => {
                localStorage.removeItem("cached_grooms");
                localStorage.removeItem("grooms_cache_timestamp");
                setAllUsers([]);
                setUsingCache(false);
